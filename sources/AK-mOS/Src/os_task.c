...
#include "os_log.h"
    ...
    /* Number of stacks from watermark to the end of stack */
    #define STACK_WATERMARK_TO_END  (20u)
    #define VALUE_WATERMARK_TO_END  (0xA3A3A3A3UL)
    #define PADDING_FOR_SAFETY      (20u) /*In stack*/
#endif

#define TASK_IDLE_ID                ((task_id_t)TASK_EOT_ID + 1u)

#define TASK_IDLE_PRI               (OS_CFG_PRIO_MAX - 1u)

#define TASK_TIMER_ID               ((task_id_t)TASK_EOT_ID)

#define TASK_TIMER_PRI              ((uint8_t)OS_CFG_TIMER_TASK_PRI) 

#define TASK_TIMER_STK_SIZE         (100u) 

#if (OS_CFG_USE_CLI == 1u)
    #include "cli.h"

    #define TASK_CLI_ID               ((task_id_t)TASK_EOT_ID + 1u)

    #define TASK_CLI_PRI              ((uint8_t)OS_CFG_CLI_TASK_PRI) 

    #define TASK_CLI_STK_SIZE         (100u) 
#endif
...
task_tcb_t * tcb_curr_ptr;
task_tcb_t * tcb_high_rdy_ptr;
...
#if (OS_CFG_USE_CLI == 1u)
    static void task_cli_func(void *p_arg)
    {
        cli_init();
        for (;;)
        {
            os_task_delay(OS_CFG_DELAY_MAX);
            /* cli_processing() */
        }
    }
#endif
...
{
#if OS_CFG_CHECK_FOR_STK_OVERFLOW == 1u
    const uint32_t * const p_limit_stk = ( uint32_t * ) tcb_curr_ptr->stk_limit_ptr;                     
                                                                                                    
    for (uint32_t i = STACK_WATERMARK_TO_END; i > 0 ; i--)
    {
        /* code */
        if((uint32_t)*(p_limit_stk - (uint32_t)i) != VALUE_WATERMARK_TO_END)
        {
                                  ...
                                  size_t stack_size)
{
    ...
    if (p_stack != NULL)
    {
        ...
        if (p_new_tcb != NULL)
        {
            ...
            p_new_tcb->stk_limit_ptr = (uint32_t *)&p_stack[STACK_WATERMARK_TO_END]; /* From watermark to the end of stack */
            for(uint16_t i = 0; i <= STACK_WATERMARK_TO_END; i++) p_stack[i] = VALUE_WATERMARK_TO_END;
#endif
        }
        else
        {
...
#if (OS_CFG_TASK_ENABLE_STACK_FILL == 1u && OS_CFG_CHECK_FOR_STK_OVERFLOW == 1u)
...
#if (OS_CFG_USE_CLI == 1u)
    p_tcb = os_task_create((task_id_t)TASK_CLI_ID,
                           (task_func_t)task_cli_func,
                           (void *)NULL,
                           (uint8_t)TASK_CLI_PRI,
                           (size_t)(OS_CFG_TASK_MSG_Q_SIZE_NORMAL),
